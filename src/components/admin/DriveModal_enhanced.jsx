...
import { 
  Loader2, Save, X, Wand2, Copy, ExternalLink, 
...
const DriveModal = ({ 
  ...
  const checkApiStatus = async () => {
    try {
      const status = await googleDriveHelper.checkGoogleDriveApiStatus();
      setApiStatus(status);
    } catch (error) {
  ...
  const generateUrls = async () => {
    ...
    const brandName = campaign.brand || 'brand';
    const userName = application.user_name || 'user';
    const userEmail = application.user_email || null;
    
    try {
      setGenerating(true);
      
      // API 연동으로 실제 폴더 및 슬라이드 생성
      const result = await googleDriveHelper.createGoogleDriveFolders(
        brandName,
        userName,
        userEmail
      );
      ...
      setFormData(prev => ({
        ...prev,
        google_drive_url: result.driveUrl,
        google_slides_url: result.slidesUrl
      }));
      
      setGenerating(false);
    } catch (error) {
      console.error('URL 자동 생성 오류:', error);
      
      // API 연동 실패 시 형식만 제공하는 URL 생성
        ...
        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          ...
          {apiStatus.initialized ? (
            <div className="mb-4 bg-green-50 border border-green-200 rounded-md p-3">
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-green-700">구글 드라이브 API가 연결되어 있습니다. 자동 생성 기능을 사용할 수 있습니다.</p>
                </div>
              </div>
            </div>
          ) : (
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-yellow-700">
                    구글 드라이브 API가 연결되어 있지 않습니다. 자동 생성 시 예시 URL만 생성됩니다.
                    {apiStatus.error && <span className="block mt-1 text-xs">{apiStatus.error}</span>}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mb-6">
            <button
              ...
              disabled={generating}
              className="inline-flex items-center px-4 py-2 border border-purple-300 shadow-sm text-sm font-medium rounded-md text-purple-700 bg-purple-50 hover:bg-purple-100 transition-colors"
            >
              {generating ? (
              ...
              {t.autoGenerate || '자동 생성'}
            </button>
            <p className="text-xs text-gray-500 mt-1">
              {apiStatus.initialized 
                ? '브랜드명과 사용자 이름을 기반으로 실제 구글 드라이브 폴더와 슬라이드를 생성합니다.'
                : '브랜드명과 사용자 이름을 기반으로 URL을 자동 생성합니다. (실제 폴더는 생성되지 않음)'}
            </p>
          </div>
